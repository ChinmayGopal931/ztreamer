...
fn range_codec(c: &mut Criterion) {
    ...
    // every record is the same size, so the only variable across indices is position
    let mut group = c.benchmark_group("decode_range_record");
    for index in [0, RANGE_SIZE as usize / 2, RANGE_SIZE as usize - 1] {
